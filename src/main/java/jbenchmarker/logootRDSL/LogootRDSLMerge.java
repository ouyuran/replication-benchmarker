 ...
 * @author mehdi urso
 */
public class LogootRDSLMerge<T> extends MergeAlgorithm {

    // nbBit <= 64
    public LogootRDSLMerge(Document doc, int r) {
        super(doc, r);

    }

    @Override
    public LogootRDSLDocument<T> getDoc() {
        return (LogootRDSLDocument<T>) super.getDoc();
    }
...
//        return lop;
    ...
    protected List<Operation> localInsert(SequenceOperation opt) throws IncorrectTraceException {
        ...
        LogootRDSLNode left = (LogootRDSLNode) walker.getPath().getLastDataNode();
        LogootRDSLNode right = (LogootRDSLNode) left.getRight(0);
        ...
        for (int cmpt = patch.size() - 1; cmpt >= 0; cmpt--) {
            ...
            lop.add(log);
            getDoc().insert(left, new LogootRDSLNode<>(c, patch.get(cmpt)), walker.getPath());
        }
        return lop;
    }
    ...
    public CRDT<String> create() {
        return new LogootRDSLMerge(getDoc().create(), 0);
    }
